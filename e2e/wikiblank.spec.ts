...
test.describe('registrazione', () => {
  test('con dati validi mostra un messaggio di successo', async ({ page }) => {
    const stamp = Date.now();

    await page.goto('/register');
    await page.locator('#username').fill(`utente${stamp}`);
    await page.locator('#email').fill(`utente${stamp}@example.com`);
  ...
  test('con un\'email già usata mostra un errore', async ({ page }) => {
    ...
    await expect(page.getByText('Un account con questa email esiste già.')).toBeVisible();
  });
});

test.describe('login', () => {
  test('con credenziali corrette porta alla partita', async ({ page }) => {
    await registerAndLogin(page);

    await expect(page).toHaveURL('/game');
  });

  test('con password sbagliata mostra un errore', async ({ page }) => {
    const stamp = Date.now();
...
test.describe('partita (utente autenticato)', () => {
  ...
  test('una parola inventata viene segnalata come assente', async ({ page }) => {
